...
public class KeyListenerEx extends JFrame {
    ...
    class MyKeyListener extends KeyAdapter {
        public void keyPressed(KeyEvent e) {
            int keyCode = e.getKeyCode(); // 키 유니코드 알아내기
            char keyChar = e.getKeyChar(); // 키 문자 값 알아내기
            keyMessage[0].setText(Integer.toString(keyCode)); // 키 코드 출력
            keyMessage[1].setText(Character.toString(keyChar)); // 키 (유니코드가 의미하는)문자 출력
            keyMessage[2].setText(KeyEvent.getKeyText(keyCode)); // 키 (사람이 인식하는 키)이름 문자열 출력
